...
class Account < ApplicationRecord
  ...
  validates :username, format: { with: /\A[^eE]+\z/, message: "not that fifth symbol" }, if: -> { local? && will_save_change_to_username? }
  validates_with UnreservedUsernameValidator, if: -> { local? && will_save_change_to_username? }
  validates :display_name, length: { maximum: 30 }, if: -> { local? && will_save_change_to_display_name? }
  validates :display_name, format: { with: /\A[^eE]+\z|\A\z/, message: "not that fifth symbol" }, if: -> { local? && will_save_change_to_display_name? }
  validates :note, length: { maximum: 160 }, if: -> { local? && will_save_change_to_note? }
  validates :note, format: { with: /\A[^eE]+\z|\A\z/, message: "not that fifth symbol" }, if: -> { local? && will_save_change_to_note? }
