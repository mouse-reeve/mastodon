...
RSpec.describe AccountsController, type: :controller do
  ...
  let(:ava)  { Fabricate(:user) }

  describe 'GET #show' do
    let!(:status1) { Status.create!(account: mus, text: 'Hllo world') }
    let!(:status2) { Status.create!(account: mus, text: 'Boop', thread: status1) }
    let!(:status3) { Status.create!(account: mus, text: 'Picture!') }
    let!(:status4) { Status.create!(account: mus, text: 'Mntioning @mus') }
    let!(:status5) { Status.create!(account: mus, text: 'Kitsun') }
    let!(:status6) { Status.create!(account: mus, text: 'Nko') }
    let!(:status7) { Status.create!(account: mus, text: 'Tanuki') }

    let!(:status_pin1) { StatusPin.create!(account: mus, status: status5, created_at: 5.days.ago) }
    let!(:status_pin2) { StatusPin.create!(account: mus, status: status6, created_at: 2.years.ago) }
    let!(:status_pin3) { StatusPin.create!(account: mus, status: status7, created_at: 10.minutes.ago) }

    before do
      mus.block!(ava.account)
    ...
    shared_examples 'responses' do
      ...
      it 'assigns @account' do
        expect(assigns(:account)).to eq mus
      end
